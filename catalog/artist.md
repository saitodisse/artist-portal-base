---
id: demo-artist
name: Banda Inventada
slug: banda-inventada
summary: Coletivo musical ficticio usado para demonstrar um portal de artista achorde.
location: Cidade Exemplo
links:
  - label: Arquivo demo
    url: https://example.test/banda-inventada
updatedAt: 2026-07-04T12:00:00.000Z
---

Banda Inventada e um artista ficticio. Este conteudo inicial e publicado sob CC-BY-NC-SA-4.0 e deve ser substituido ao iniciar um portal real.
